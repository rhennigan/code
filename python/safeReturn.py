...
# Task 2: Compute the spacecraft's acceleration due to gravity
	###Your code here.
    to_earth = position
    to_moon = position - moon_position(time)
    earth_acc = - gravitational_constant * earth_mass / numpy.linalg.norm(to_earth)**3.0 * to_earth
    moon_acc = - gravitational_constant * moon_mass / numpy.linalg.norm(to_moon)**3.0 * to_moon
...
def apply_boost():
    ...
    while current_time < total_duration:
        ...
        if current_time >= 101104 and not mcc2_burn_done:
            direction = velocity / numpy.linalg.norm(velocity)
            dv = -7.04 * direction
            velocity += dv
            mcc2_burn_done = True
        ...
        ###Your code here.
        lte = numpy.linalg.norm(positionE - position) + total_duration * numpy.linalg.norm(velocityE - velocity)
