 ...
 * Richard Hennigan
 * Thu Oct 2 14:32:07 EDT 2014
 *
 * Types:
 *   TYPE_clist_t - polymorphic circular linked list
 *
 * Functions:
 *   +> T1_T2_clist_map()
 *   +> TYPE_clist_app()
...
#ifndef _LIB_DATA_CIRCULAR_LINKED_LIST_H
#define _LIB_DATA_CIRCULAR_LINKED_LIST_H
...
#define def_clist_find(TYPE)							\
  bool										\
  TYPE##_clist_find (TYPE##_clist_t * list, TYPE obj)				\
  {										\
    TYPE##_clist_t *start = list;						\
...
#define def_clist_count(TYPE)							\
  int										\
  TYPE##_clist_count (TYPE##_clist_t * list, TYPE obj)				\
  {										\
    int count = 0;								\
    TYPE##_clist_t *start = list;						\
    TYPE##_clist_t *current = list;						\
...
#define def_clist_map(T1, T2)							\
  T2##_clist_t *							        \
  T1##_T2##_clist_map (T1##_clist_t * list, T2 (*f) (T1))		        \
  {										\
    if (list == EMPTY)								\
      {										\
	return EMPTY;								\
