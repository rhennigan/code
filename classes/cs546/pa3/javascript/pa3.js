...
(function() {
  var Line, Viewer, callback, clear, createMeshLines, createSVGLine, err, generalizedTransformation, getVertexMean, getVertexRanges, isometricProjection, load, loadObject, main, orthoProj, parseFace, parseVertex, rescaleVertices, rotate, transformVertices, transformationMatrix, union,
  ...
  getVertexMean = function(vertices) {
    var k, len1, n, v, xs, ys, zs;
    xs = ys = zs = 0;
    n = vertices.length;
    for (k = 0, len1 = vertices.length; k < len1; k++) {
      v = vertices[k];
  ...
  main = function() {
    var SVG_SIZE, object3D, perspective, reset, rotation, scale, shear, translation;
    SVG_SIZE = Math.min(window.innerWidth - 30, window.innerHeight - 175) / 2;
    document.getElementById('imgTbl').width = 2 * SVG_SIZE;
    object3D = load('Cube');
    scale = {
