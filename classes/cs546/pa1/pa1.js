...
(function() {
  var BTN_BACKGROUND_DISABLED, BTN_BACKGROUND_ENABLED, BTN_TEXT_DISABLED, BTN_TEXT_ENABLED, Circle, Color, DrawingCanvas, Ellipse, Fractal, FractalCanvas, Geometry, Line, Polygon, Polyline, Rectangle, drawingCanvas, first, firstScaled, fractalCanvas, last, lastScaled, nu, nv, points, polygon, polyline, polylineDistance, pt, rotatedPoints, s, scaledPoints, segment, segmentDistance, segments, t1, t2, theta, translatedPoints, u, ui, v,
  ...
  scaledPoints = (function() {
    ...
    for (j = 0, len1 = points.length; j < len1; j++) {
      pt = points[j];
      s = Geometry.prototype.vecSub(pt, first);
      results.push(Geometry.prototype.vecSMul(segmentDistance, Geometry.prototype.normalize(s)));
    }
    return results;
  })();
  ...
  translatedPoints = (function() {
    var j, len1, results;
    results = [];
    for (j = 0, len1 = rotatedPoints.length; j < len1; j++) {
      pt = rotatedPoints[j];
      results.push(Geometry.prototype.vecAdd(pt, segment.pt1));
    }
    return results;
  })();
