...
let ElapsedTime(startTime:int64, task:string) =
    ...
    #if COMPILED
    Debug.Log("Time to " + task + ": " + (elapsed/10000L |> string) + " ms")
    #else
...
let EdgeFill =
    let vs = ARRAY_SIZE |> int
    let b = vs * vs - vs
    seq { for i in 0 .. 2 .. (vs - 3) do
           yield! seq [ vs * i;             //Left side
                        vs * (i + 2); 
                        vs * (i + 1);

                        vs * i;             //Left side reversed orientation
                        vs * (i + 1);
                        vs * (i + 2); 
                        ...
                        vs * (i + 3) - 1;   //Right side reversed orientation
                        vs * (i + 2) - 1;
                        vs * (i + 1) - 1;
                        ...
                        b + i + 1;          //Bottom reversed orientation
                        b + i + 2;
                        b + i;
                      ]
    } |> Array.ofSeq
...
let UV = let vs = ARRAY_SIZE |> int
         let w = float32 vs - 1.0f
         let h = float32 vs - 1.0f
         seq { for i in 0.0f .. h do
                for j in 0.0f .. w do
                  let r = j / w
                  let u = i / h
                  yield Vector2(r, u)
...
type EvaluationQueue () = 
    ...
    member this.Evaluate () =
        ...
        | true  -> ()
        | false -> if queue.Count > 0
                   then currentEvaluator <- queue.Dequeue ()
...
let CalculateMeshTangents(mesh:Mesh) =
    ...
    for a in 0..3..(triangleCount-1) do
        ...
        let sdir = Vector3((t2 * x1 - t1 * x2) * r, 
                           (t2 * y1 - t1 * y2) * r, 
                           (t2 * z1 - t1 * z2) * r)

        let tdir = Vector3((s1 * x2 - s2 * x1) * r, 
                           (s1 * y2 - s2 * y1) * r, 
                           (s1 * z2 - s2 * z1) * r)
    ...
    for a in 0..(vertexCount-1) do
        ...
        tangents.[a].w <- if (Vector3.Dot(Vector3.Cross(n, t), tan2.[a]) < 0.0f) 
                          then -1.0f else 1.0f
...
let Subdivide(key:Key, rad:float32, amp:float32) =
    ...
    | true  -> List.iter BuildGameObject ch
               Disable(key)
               ch
    | false -> let bl, br, tl = CornerCache.[key]
...
let Merge(key:Key) =
    ...
    | false -> key
    | true  -> Neighbors(key) |> List.iter Disable
               let p = Parent(key)
               Enable(p)
               p
...
let PrintKey(key:Key) =
    ...
    #if COMPILED
    Debug.Log(pID + "_" + f + "_" + cID)
    #else
    printfn "%s" (pID + "_" + f + "_" + cID)
    #endif
